...
std::string sign_transaction(const std::string &transactionHash, const std::string &privateKey)
{
    ...
    std::vector<unsigned char> rawHash = base64_decode(transactionHash);

    std::vector<unsigned char> keyBytes = hex_to_bytes(privateKey);

    std::vector<unsigned char> rawSignature = sign_with_ED25519_raw_key(keyBytes, rawHash);
    ...
    std::string signatureBase64 = base64_encode(rawSignature);
